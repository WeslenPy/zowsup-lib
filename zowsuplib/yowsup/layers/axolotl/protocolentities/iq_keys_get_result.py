...
from zowsuplib.axolotl.ecc.djbec import DjbECPublicKey
import binascii
import sys
...
class ResultGetKeysIqProtocolEntity(ResultIqProtocolEntity):
    ...
    @staticmethod
    def fromProtocolTreeNodeCheckNum(node):
        ...
        userNodes = listNode.getAllChildren()        
        for userNode in userNodes:
            missing_params = []
            preKeyNode = userNode.getChild("key")

            if preKeyNode is None:
                entity.userActiveMap[userNode["jid"]] = None
            else:
                entity.userActiveMap[userNode["jid"]] = int(userNode.getAttributeValue("t"))
