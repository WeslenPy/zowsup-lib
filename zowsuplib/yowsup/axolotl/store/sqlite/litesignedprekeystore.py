from zowsuplib.axolotl.state.signedprekeystore import SignedPreKeyStore
...
class LiteSignedPreKeyStore(SignedPreKeyStore):
    ...
    def storeSignedPreKey(self, signedPreKeyId, signedPreKeyRecord):
        ...
        self.dbConn.commit()
        ts = signedPreKeyRecord.getTimestamp()
        q = "INSERT INTO signed_prekeys (prekey_id, timestamp, record) VALUES(?,?,?)"
        cursor = self.dbConn.cursor()
        record = signedPreKeyRecord.serialize()
        cursor.execute(q, (signedPreKeyId, ts, buffer(record) if sys.version_info < (2,7) else record))
        self.dbConn.commit()
